...
void LogWindow::Draw()
{
    ...
    if ( ImGui::BeginChild( "scrolling", ImVec2{ 0.f, 0.f }, false,
                            ImGuiWindowFlags_HorizontalScrollbar ) )
    {
        ...
        for ( int i = 0; i < m_TextOffsets.Size; ++i )
        {
            ...
            if ( ( bIsInfo && !m_bShowInfo ) || ( bIsWarn && !m_bShowWarn )
                 || ( bIsError && !m_bShowError ) )
                continue;
...
void LogWindow::GetLogs()
{
    const auto& logs{ JADE_GET_LOGS() };
