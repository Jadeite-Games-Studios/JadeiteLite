...
std::shared_ptr<Shader> ShaderLoader::CreateFromMemory( const char* vertexShader,
                                    ...
                                    const std::string& fragmentShader )
{
    ...
    {
        JADE_ERROR( "Failed to Link Shaders!" );
        return 0;
    }
...
GLuint ShaderLoader::CompileShader( GLuint shaderType, const std::string& filepath )
{
    ...
    if ( ifs.fail() )
    {
        JADE_ERROR( "Shader Failed to open [{}]", filepath );
        return 0;
    }
    ...
    {
        JADE_ERROR( "Failed to compile shader [{}]", filepath );
        return 0;
    }
...
GLuint ShaderLoader::CompileShader( GLuint type, const char* shader )
{
    ...
    {
        JADE_ERROR( "Failed to compile shader from memory!" );
        return 0;
    }
...
bool ShaderLoader::CompileSuccess( GLuint shader )
{
    ...
    if ( status != GL_TRUE )
    {
        ...
        JADE_ERROR( "Shader Compilation failed: {}", std::string{ errorLog } );
...
bool ShaderLoader::IsProgramValid( GLuint program )
{
    ...
    if ( status != GL_TRUE )
    {
        ...
        JADE_ERROR( "Shader's Program failed to link: {}", std::string{ errorLog } );
        return false;
    }
