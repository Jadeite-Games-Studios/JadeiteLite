                                                ...
                                                const std::string& texturePath )
{
    ...
    case Texture::TextureType::PIXEL:
        bLoadSuccessful = LoadTexture( texturePath, id, width, height, false );
        break;
    case Texture::TextureType::BLENDED:
        bLoadSuccessful = LoadTexture( texturePath, id, width, height, true );
        break;
    case Texture::TextureType::ICON:
        JADE_ASSERT( false && "Icon Texture creation not implemented" );
        break;
    default:
        JADE_ASSERT( false
                     && "The current type is not defined, Please use a defined texture type!" );
        return nullptr;
    }

    return bLoadSuccessful ? std::make_shared<Texture>( id, width, height, type, texturePath )
                           : nullptr;
}

std::shared_ptr<Texture> TextureLoader::Create( Texture::TextureType type, int width, int height )
{
    JADE_ASSERT( type == Texture::TextureType::FRAMEBUFFER && "Must be framebuffer type" );

    if ( type != Texture::TextureType::FRAMEBUFFER )
    {
        JADE_ERROR( "Failed to create texture for frame buffer. Incorrect type passed in!" );
        return nullptr;
    }
...
std::shared_ptr<Texture> TextureLoader::CreateFromMemory( const unsigned char* imageData,
                                                          size_t length, bool blended )
{
    GLuint id;
    int width, height;
...
bool TextureLoader::LoadTexture( const std::string& filepath, GLuint& id, int& width, int& height,
                                 bool blended )
{
    int channels = 0;
    ...
    if ( !image )
    {
        JADE_ERROR( "SOIL failed to load image [{0}] -- {1}", filepath, SOIL_last_result() );
        return false;
    }
...
bool TextureLoader::LoadTextureFromMemory( const unsigned char* imageData, size_t length,
                                           GLuint& id, int& width, int& height, bool blended )
{
    id = SOIL_load_OGL_texture_from_memory( imageData, length, SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID,
                                            NULL );

    if ( id == 0 )
    {
        JADE_ERROR( "Failed to load texture from memory." );
        return false;
    }
